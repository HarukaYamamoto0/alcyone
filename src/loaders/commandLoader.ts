 ...
 *
 * @param {URL | null} path - The directory path where command files are located. If null, defaults to "../commands/"
 * relative to the current module.
 * @return {Promise<Collection<string, BaseCommand>>} A promise that resolves to a collection of command names mapped
 * to their respective command objects.
 */
async function commandLoader(path: URL | null = null): Promise<Collection<string, BaseCommand>> {
  ...
  for (const category of categoryNames) {
    ...
    const commandFiles = readdirSync(fileURLToPath(categoryPath)).filter(
